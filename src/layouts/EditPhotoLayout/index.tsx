...
interface IEditPhotoLayoutProps {
	editPage?: boolean;
}

function EditPhotoLayout({ editPage }: IEditPhotoLayoutProps): JSX.Element {
	useAuthGuard();
	...
	const handleGetPhotoData = async (): Promise<void> => {
		try {
			const data = await photoService.getPhotoById(photoId).then(r => r.data);
	...
	}, [handleGetPhotoData, editPage])

	return (
		<>
			<Seo title={editPage ? 'Edit Photo' : 'Upload Photo'} />
			<InternalPageLayout
				pageTitle={editPage ? 'Edit a Photo' : 'Upload a New Photo'}
				pageDescription={
					editPage ? 'Edit the details of a photo' : 'Save a new photo'
				}
			>
